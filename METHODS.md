...
- **Reward Factor**: Goals are weighted based on the initial SPI ranking of the opponent. The reward factor multiplies goals scored by an adjustment factor, times the opponent team's SPI divided by the minimum SPI. The goal of this adjustment is to effectively weight goals, based on the difficulty implied in scoring them. The reward factor has an adjustment factor, that is calculated to ensure that the weighted goals are within the range of the 95th percentile of goals scored in a match, once a levelling factor is considered.
...
- **Low SPI Teams Matches**: The model separates matches between teams that both have an SPI under the 7th percentile, to avoid creating xG and xGA metrics that give too much importance to these matches. Goals against in such matches are set to the 95th percentile goals scored per match, and goals for to the inverse of the 95th percentile. This means that teams under the 7th percentile must play and score against better ranked teams, in order to make progress in the ranking. This might seem too unfair, but it reflects the difficulty in practice of making progress in the ranking, once a team is in the bottom end, in terms of SPI. This also avoids that teams that only play teams in the bottom of the ranking get unfairly high offensive and unfairly low defensive metrics. The cutoff percentile (the 7th) is calculated to ensure that no teams with xG under 1 also get xGA under 1, which indicates the imbalance that arises for teams under the 7th percentile who mostly play teams under the 7th percentile SPI.
